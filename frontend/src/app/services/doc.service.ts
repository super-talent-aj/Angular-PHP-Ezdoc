...
export class DocService {
  ...
  getDocByClientForm(client_id, form_id) {
    let url = Common.BASE_URL + "/api/v1/getDoc?client_id=" + client_id + "&form_id=" + form_id + "&token=" + Common.getUser().token;
    return this.http.get(url)
      .map((response: Response) => response.json())
      .do(data => data)
      .catch(ErrorHandler.handleError);
  ...
  approveDismissDoc(doc_id, approve) {
    let url = Common.BASE_URL + "/api/v1/docs/approveDismiss?doc_id=" + doc_id + "&approve=" + approve + "&token=" + Common.getUser().token;
    return this.http.get(url)
      .map((response: Response) => response.json())
      .do(data => data)
      .catch(ErrorHandler.handleError);
